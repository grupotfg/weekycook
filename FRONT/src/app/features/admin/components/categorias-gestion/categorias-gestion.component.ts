...
export class CategoriasGestionComponent implements OnInit {
  ...
  guardar() {
    if (this.isEditing && this.selectedCat.id) {
      this.catService.update(this.selectedCat.id, this.selectedCat).subscribe(() => this.finalizar());
    } else {
      this.catService.create(this.selectedCat).subscribe(() => this.finalizar());
    }
  }
  ...
  eliminar(id: number) {
    if (confirm('¿Estás seguro de eliminar esta categoría?')) {
      this.catService.delete(id).subscribe({
        ...
        error: () => alert('Error: No se puede eliminar una categoría con recetas asociadas.')
      });
    }
  }
