...
export class IngredientesGestionComponent implements OnInit {
  ...
  get ingredientesFiltrados() {
    return this.ingredientes.filter(ing => 
      ing.nombre.toLowerCase().includes(this.searchText.toLowerCase())
    );
  }
  ...
  private resetIng(): Ingredient {
    return { nombre: '', unidadBase: 'gr', caloriasPorUnidad: 0, proteinasPorUnidad: 0, grasasPorUnidad: 0, hidratosPorUnidad: 0 };
  }
}